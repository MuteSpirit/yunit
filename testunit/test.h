...
#define test(name)\
    test_(name)\
    registerTest(name, TestSourceLine())\
    testBodyDef(name)
...
#define _test(name)\
    test_(name)\
    registerIgnoredTest(name, TestSourceLine())\
...
#define isTrue(condition)\
	if(!TESTUNIT_NS::cppunitAssert(condition))\
		TESTUNIT_NS::throwException(TestSourceLine(), #condition)

#define isFalse(condition)\
	if(TESTUNIT_NS::cppunitAssert(condition))\
		TESTUNIT_NS::throwException(TestSourceLine(), #condition " != false", false)

#define areEq(expected, actual)\
	if(!TESTUNIT_NS::cppunitAssert((expected), (actual)))\
		TESTUNIT_NS::throwException(TestSourceLine(), (expected), (actual), true)
