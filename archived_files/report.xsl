...
<xsl:template match="ErrorTests">
    <H2>ErrorTests</H2>
    <xsl:choose>
	<xsl:when test="ErrorTest">
		<TABLE>
			<TR>
				<TH>id</TH>
				<TH>Name</TH>
		  	...
		  	<xsl:apply-templates select="ErrorTest"/>
		</TABLE>
	</xsl:when>
	<xsl:otherwise>
	    <SPAN class="good">No error test.</SPAN>
	</xsl:otherwise>
    </xsl:choose>
</xsl:template>

<xsl:template match="ErrorTest">
	<TR>
	<TD align="right" valign='top'><xsl:value-of select="@id"/></TD>
	<TD valign='top'><xsl:apply-templates select="Name"/></TD>
	<TD valign='top'><xsl:apply-templates select="FailureType"/></TD>
...
<xsl:template match="IgnoredTests">
    <H2>IgnoredTests</H2>
    <xsl:choose>
	<xsl:when test="IgnoredTest">
	<TABLE>
		<TR>
			<TH>id</TH>
			<TH>Name</TH>
		</TR>
	  	<xsl:apply-templates select="IgnoredTest"/>
	</TABLE>
	</xsl:when>
	<xsl:otherwise>
	    <SPAN class="good">No ignored test.</SPAN>
	</xsl:otherwise>
    </xsl:choose>
</xsl:template>
...
<xsl:template match="Statistics">
	...
	<TABLE>
		...
		<TR>
			<TD>Ignores</TD>
			<TD align="right"><xsl:value-of select="Ignores"/></TD>
		</TR>
